...
// commands have run. The entry path and run arguments remain argv items rather
// than being interpolated into shell source.
const pythonRuntimeBootstrap = `project_root=$PWD; package_root=; if [ -n "${PIP_TARGET:-}" ] && [ -d "$PIP_TARGET" ]; then package_root=$PIP_TARGET; elif [ -d /persist/pip-packages ]; then package_root=/persist/pip-packages; fi; if [ -n "$package_root" ]; then export PYTHONPATH="$package_root:$project_root"; else export PYTHONPATH="$project_root"; fi; exec python3 "$@"`
