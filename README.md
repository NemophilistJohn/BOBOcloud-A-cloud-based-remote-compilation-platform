...
Renderer localization is a typed, lifecycle-owned service. Only the native host adapter reads the preload bridge and exposes a private `host.languagePacks` capability; the compatibility adapters register private `workbench.i18n` and `workbench.languagePacksPanel` services while projecting only the historical `BOBO.i18n` instance and three-method `BOBO.languagePacksPanel` facade. Language-pack DTOs are normalized at the renderer boundary, host change events are treated only as invalidation hints, and locale changes and panel renders use latest-wins commits so disposed or superseded requests cannot update the DOM. The panel owns and releases its DOM and localization subscriptions, and normalizes duplicate pack identifiers in linear time.

Renderer theme selection is a typed, lifecycle-owned private `workbench.theme` service with no preload capability. Its compatibility adapter projects only the historical seven-method `window.themeManager` facade and applies the saved theme synchronously before later workbench modules start. Theme summaries are defensive DTOs, change listeners are disposable and error-isolated, and each built-in Monaco theme is defined at most once per service lifetime.

Renderer confirmations use a typed, lifecycle-owned private `workbench.confirm` service. It preserves the historical lazy DOM, FIFO request queue, focus timing, selectors, and callable `BOBO.confirm` facade while giving boolean and details responses distinct TypeScript contracts. Platform disposal cancels timers, removes listeners and DOM, preserves a selection already made during closing, and fail-closes active or queued requests so no confirmation promise remains pending; downloaded plugins receive no confirmation service.

The rclone selector is also a typed, lifecycle-owned private workbench service. Its selection requests carry only sender-bound scan ids and opaque candidate ids; native confirmation, executable identity checks, content-addressed copying, and activation remain in Electron main. Scan and status updates use independent latest-wins epochs, renderer listeners are released with the platform lifecycle, and resize positioning is frame-coalesced without caching executable scans.
