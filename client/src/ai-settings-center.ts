...
// The DOM contract and seven-key BOBO facade remain compatible with the
// historical implementation. Browser and sibling-module access is supplied
// through typed dependencies and the private renderer service registry.

import { DisposableStore, toDisposable } from '../renderer/core/disposable.js';
import type { AiSettingsCenterAgentButtonPort, AiSettingsCenterDependencies, AiSettingsCenterFacade, AiSettingsCenterDraft, AiSettingsCenterI18nPort, AiSettingsCenterSchemaPort, AiSettingsCenterService, AiSettingsCenterAgentWorkbenchPort } from '../types/ai-settings-center';
import type { AiOperationResultDto, AiProfileDto, AiPurpose, AiSettingsDto } from '../types/ai-service';
import type { Dispose } from '../types/lifecycle';
import type { RendererIconsFacade } from '../types/icons';

export const AI_SETTINGS_CENTER_SERVICE_ID = 'workbench.aiSettingsCenter' as const;

interface InternalBobo {
  i18n?: AiSettingsCenterI18nPort;
  aiSettingsSchema?: AiSettingsCenterSchemaPort;
  aiService?: ReturnType<AiSettingsCenterDependencies['getAiService']>;
  confirm?: ReturnType<AiSettingsCenterDependencies['getConfirm']>;
  icons?: RendererIconsFacade;
  aiAgentButton?: AiSettingsCenterAgentButtonPort;
  agentWorkbench?: AiSettingsCenterAgentWorkbenchPort;
}

export function createAiSettingsCenterService(dependencies: AiSettingsCenterDependencies): AiSettingsCenterService {
  ...
  const setTimeout = (callback: () => void, delayMs: number): number => {
    ...
    const wrapped = (): void => { if (handle !== null) ownedTimers.delete(handle); callback(); };
    handle = dependencies.setTimer(wrapped, delayMs);
    if (handle !== null) ownedTimers.add(handle);
    return handle;
  };
  const clearTimeout = (timer: number): void => {
    ownedTimers.delete(timer);
    try { dependencies.clearTimer(timer); } catch (_) { /* best effort */ }
  };
  ...
  Object.defineProperties(BOBO, {
    ...
    aiSettingsSchema: { enumerable: true, get: () => dependencies.getSchema() || undefined },
    aiService: { enumerable: true, get: () => dependencies.getAiService() || undefined },
    confirm: { enumerable: true, get: () => dependencies.getConfirm() || undefined },
    ...
    agentWorkbench: { enumerable: true, get: () => dependencies.getAgentWorkbench() || undefined }
  });
  ...
  let draft: AiSettingsCenterDraft | null = null;
  let dirty = false;
  let saving = false;
  let autoSaveTimer: number | null = null;
  let activeSavePromise: Promise<AiOperationResultDto | undefined> | null = null;
  let draftRevision = 0;
  let saveGeneration = 0;
  const profileTestGeneration: Record<string, number> = Object.create(null);
  let previousFocus: HTMLElement | null = null;
  let editingProfileId = '';
  let editingPurpose: AiPurpose = 'chat';
  let profileEditorDraft: AiProfileDto | null = null;
  let profileEditorRaw: Record<string, unknown> | null = null;
  let profileEditorProviderDrafts: Record<string, Record<string, unknown>> = Object.create(null);
  let profileEditorAdvancedOpen = false;
  let profileEditorRenderGeneration = 0;
  let unsubscribeLocale: Dispose | null = null;
  ...
  function ensureDOM() {
    ...
    modal.querySelectorAll('[data-ai-tab]').forEach(function(tab) {
      listen(tab, 'click', function() { switchTab(tab.dataset.aiTab); });
    ...
    listen(document.getElementById('ai-control-close'), 'click', requestClose);
    listen(document.getElementById('ai-control-cancel'), 'click', requestClose);
    listen(document.getElementById('ai-control-save'), 'click', save);
    listen(modal, 'click', function(event) { if (event.target === modal) requestClose(); });
    listen(modal, 'keydown', function(event) {
  ...
  const facade: AiSettingsCenterFacade = {
    init: ensureDOM,
    open,
    close: requestClose,
    save,
    switchTab,
    isDirty: () => dirty,
    getDraft: () => draft ? clone(draft) as AiSettingsDto : null
  };
  ...
  function dispose(): void {
    ...
    if (unsubscribeLocale) {
      try { unsubscribeLocale(); } catch (_) { /* best effort */ }
