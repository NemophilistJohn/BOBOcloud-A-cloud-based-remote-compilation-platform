// SVG icon system for BOBOCLOUD Editor.
// All icons use a 16x16 viewBox, stroke-based, currentColor.
import type {
  RendererIconsFacade,
...
function icon(paths: string, size = 14): string {
  return '<svg width="' + size + '" height="' + size +
    '" viewBox="0 0 16 16" fill="none" style="flex-shrink:0">' + paths + '</svg>';
}

const ICONS: RendererIconsFacade = {
  // ── Actions ──
  plus: icon(
...
export const icons: RendererIconsSnapshot = Object.freeze(ICONS);
