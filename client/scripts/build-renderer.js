...
const TERMINAL_UI_OUTPUT_FILE = path.join(OUTPUT_DIRECTORY, 'bobo-terminal-ui.js');

function readOrderedImports(source) {
  return Array.from(source.matchAll(/^import\s+(?:[^'";]+?\s+from\s+)?['"]([^'"]+)['"];?\s*$/gm), (match) => match[1]);
}
...
async function buildRenderer(options = {}) {
  ...
  const result = await esbuild.build({
    ...
    entryPoints: {
      ...
      'bobo-terminal-ui': TERMINAL_UI_ENTRY
    },
  ...
  const terminalUiEntrySource = await fs.readFile(TERMINAL_UI_ENTRY, 'utf8');
  const orderedModules = readOrderedImports(entrySource);
  const orderedAiUiModules = readOrderedImports(aiUiEntrySource);
  const orderedTerminalUiModules = readOrderedImports(terminalUiEntrySource);
  ...
  return { mode, outputFile, aiUiOutputFile, terminalUiOutputFile, metadataFile, manifestFile, result };
}
