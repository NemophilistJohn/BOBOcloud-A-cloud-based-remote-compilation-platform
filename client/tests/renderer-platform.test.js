...
test('renderer platform bootstrap exposes the precise runtime contract without a cast adapter', () => {
  const source = [
    ...
    "created.services.require('workbench.settings').open('local');",
    "created.services.require('workbench.accountProfile').renderActivity();",
    ...
    '// @ts-expect-error The unified settings shell is private to the workbench.',
    "created.services.getForPlugin('workbench.settings');",
    '// @ts-expect-error The account profile UI is private to the workbench.',
    "created.services.getForPlugin('workbench.accountProfile');",
