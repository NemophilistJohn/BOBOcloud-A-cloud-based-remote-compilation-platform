...
test('workspace editor settings apply on open and refresh after settings.json changes', async () => {
  ...
  try {
    ...
    })).toEqual({ language: 'html', tabSize: 2, insertSpaces: false, wordWrap: 'bounded', leaked: false });
    ...
    }), { timeout: 10000 }).toEqual({ language: 'javascript', tabSize: 6, insertSpaces: true, wordWrap: 'on' });
  } finally {
